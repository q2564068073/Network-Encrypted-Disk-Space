        ...
        try:
            ...
            if message:
                ...
                if message_type == 'register':
                    ...
                    send_back(safe_socket,str(message_return))
                    message_return_2 = safe_socket.recv_decrypt().decode('utf-8')
                    if message_return_2 == 'ok':
                        with open(f'./file/{username}/{filename}', 'rb') as f:
                            data = f.read()
                            safe_socket.send_encrypt(data)
                    ...
                    group_key = message_parts[2]
                    message_return = create_group(group_name,group_key)
                    send_back(safe_socket,str(message_return))
                elif message_type == 'login_group':
                    group_name = message_parts[1]
                    group_key = message_parts[2]
                    message_return = login_group(group_name,group_key)
                    send_back(safe_socket,str(message_return))
                elif message_type == 'get_group_list':
                    group_name = message_parts[1]
                    message_return = get_group_list(group_name)
                    send_back(safe_socket,str(message_return))
                elif message_type == 'upload_group':
                    group_name = message_parts[1]
                    filename = message_parts[2]
                    message_return = upload_group(group_name, filename)   #upload_group|010|res
                    send_back(safe_socket, str(message_return))
                    #记得写个数据库函数把前面的三个关键词入表
                    if message_return == 'ok':
                        file = safe_socket.recv_decrypt() #收到加密后的文件
                        save_file(group_name,filename,file)
                        #print(file)
                    #存储加密文件，记得用filename命名
                elif message_type == 'download_group':
                    group_name = message_parts[1]
                    filename = message_parts[2]
                    '''
                    message_return = download_group(group_name,filename)
                    send_back(safe_socket,str(message_return))
                    message_return_2 = safe_socket.recv_decrypt().decode('utf-8')
                    if message_return_2 == 'ok':
                    '''
                    with open(f'./file/{group_name}/{filename}', 'rb') as f:
                        data = f.read()
                        safe_socket.send_encrypt(data)                 
        except:
