...
#服务器端检测username和filename提取出对应的密码，进行比对
#如果密码一样的话，就在仓库里面找到对应的文件，随后发送出去
...
def create_group(group_name,group_key):
    k = create_group_in_sql(group_name,group_key)
    if k == 0:
        return 0 #群名已经存在了
    else:
        return 2

def login_group(group_name,group_key):
    k = login_group_in_sql(group_name,group_key)
    return k

def get_group_list(group_name):
    k = get_group_list_in_sql(group_name)
    print(k)
    if k==0:
        return 0
    else:
...
def upload_group(group_name, filename):
    k = upload_group_in_sql(group_name, filename)
    print(k)
    if k == 0:
        return 0
    elif k == 2:
...
def download_group(group_name, filename):
    k = download_group_in_sql(group_name, filename)
    if k==1:
        return 1
    elif k==0:
        return 0
